...
【誤字脱字・表記のチェック（厳密に・公平に）】
- **基準は「画像に実際に書かれている文字」だけにしてください。** 正しく書けている字を「忘れている」「抜けている」と決めつけて指摘しないでください。特に促音（っ）は、画像に明らかに書かれていないと判断できる場合のみ指摘し、正しく「っ」が書けている場合は指摘しないでください。
- 画像に書かれた文字を正しい表記と照らし合わせ、**実際に誤って書かれているものだけ**を指摘してください：
  - 似た字の誤り（き↔さ、め↔ぬ、わ↔れ、あ↔お など）は、画像を注意深く見て、誤りがあれば**必ず**すべて指摘してください。見逃さないでください。
  - 漢字の形の誤り（トメ・ハネ・画数。例：「目」の横線の本数、「田」の縦横の本数）も同様に、誤りがあれば必ず指摘してください。
  - 送り仮名の誤り、同音異義語の誤用、文法・表現の不自然さ。
- 「誤字脱字はなし」「特になし」と書くのは、**本当に誤りが一つもない場合だけ**にしてください。逆に、正しく書けているものを誤りと指摘しないでください。

【ルール評価・改善点】
- 以下で「作文ルール」が指定されている場合、そのルールが守られているかを**必ず**評価し、守られていないルールは ruleEval と improvements の**両方**に記載してください。改善点に「とくになし」と書くのは、誤字脱字がなく**かつ**指定されたルールをすべて満たしている場合**のみ**にしてください。
- 良い点はしっかり具体的に伝えてください。
...
${theme ? `\n【テーマ】${theme}` : ""}
${rules.trim() ? `\n【作文ルール（守られているか必ず評価し、違反は ruleEval と improvements に必ず書く）】\n${rules.trim()}` : ""}
`;
